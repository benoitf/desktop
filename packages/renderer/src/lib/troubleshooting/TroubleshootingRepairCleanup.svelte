...
async function cleanup() {
  try {
    ...
    cleanupFailures = [];
    await window.cleanupProviders(providerIdsWithCleanup);
  } catch (e) {
    console.error(e);
...
<div class="flex flex-row items-center">
  <div>
    <div class="text-gray-700 flex flex-row items-center">
      <Fa size="18" class="pr-2 text-gray-700" icon="{faBroom}" /> Clean / Purge data
    </div>
    <div class="text-gray-900 text-xs flex flex-row items-center">
      <Fa class="pl-7 pr-1" size="9" icon="{faWarning}" />Proceeding with this action may result in data loss, including
